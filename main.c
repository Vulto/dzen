...
main(int argc, char *argv[]) {
	...
	for(i = 1; i < argc; i++)
		if(!strncmp(argv[i], "-l", 3)){
			...
			eprint("usage:	-v < show version > \n"
										"				-p < display message time in seconds >\n"
										"				-m v|h\n"
										"				-ta < l|c|r >\n"
										"				-sa < l|c|r >\n"
                   	"				-x < horizontal position in pixels >\n"
				   					...
				   					"				-e < string >\n"
										...
										"				-bg <backgroung color>\n"
										"				-fg <foreground color>\n"
				   					"				-geometry <geometry string example: 100x50>\n"
										"				-expand <left or right>\n"
										"				-dock < make dzen appears beside the status bar >\n"
				   					"				-title-name <title appears on dwm bar>\n"
										"				-slave-name <string>\n"
#ifdef DZEN_XINERAMA
				   "             [-xs <screen>]\n"
#endif
